        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
          {notificationSections.map((section) => (
            <div key={section.title}>
              <h3 className="text-sm sm:text-base font-semibold text-foreground mb-3 sm:mb-4">{section.title}</h3>
              <div className="space-y-3 sm:space-y-4">
                {section.items.map((item) => (
                  <div key={item.key} className="flex items-center justify-between">
                    <div className="flex-1">
                      <p className="text-sm sm:text-base font-medium text-foreground">{item.label}</p>
          ...
          <div className="flex space-x-3 pt-4 sm:pt-6">
            ...
            <button
              onClick={handleSave}
              className="flex-1 py-2.5 sm:py-3 bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700 text-white rounded-lg sm:rounded-xl text-sm sm:text-base font-medium transition-all duration-200 hover:scale-[0.98] shadow-lg hover:shadow-xl"
            >
              Save Settings
            </button>
          </div>
